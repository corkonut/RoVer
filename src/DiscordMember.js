...
class DiscordMember {
  ...
  async verify (options) {
    ...
    if (this.user.bot) {
      return status({
        status: false,
        error: ':robot: Verification cannot verify bots.'
      })
    }

    // Ignore users with this specific role (to give server owners more power)
    if (this.member.roles.find(role => role.name === 'Verification Bypass')) {
      return status({
        status: false,
        error: ':octagonal_sign: Verification cannot act on users with the "Verification Bypass" role.',
        nonFatal: true
      })
    }
    ...
    if (!this.member.manageable) {
      errorAppend = "\n\nVerification's position in the role list is below that of this user. Please have a server admin drag Verification's role above all other roles in order to fix this problem."
    }
    ...
    if (data.status === 'ok') {
        ...
        if (!this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "There was an error while trying to assign the verified role! Ensure Verification's role is above it." + errorAppend
            })
          }
        }
      }
        ...
        if (this.member.roles.has(role) && this.server.roles.has(role)) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: "There was an error while trying to remove the verified role! Ensure Verification's role is above it." + errorAppend
            })
          }
        }
      }
      ...
      if (
        ...
        !this.member.roles.find(role => role.name === 'Verification Nickname Bypass')
      ) {
        ...
        if (this.member.displayName !== nickname) {
          try {
            ...
            return status({
              status: false,
              nonFatal: true,
              error: this.member.guild.ownerID === this.member.id ? "Sorry, Discord doesn't allow bots to change the server owner's nickname. Please manually update your nickname. Or don't, I'm just an error message." : "Verification doesn't have permission to change that user's nickname." + errorAppend
            })
          }
        }
      }
