...
class TransferPlus extends Command {
  constructor (client) {
    super(client, {
      ...
      description: 'Transfer Verification Plus ownership to someone else',
      userPermissions: [],
      guildOnly: false,
  ...
  async fn (msg, args) {
    if (!this.discordBot.isPremium()) {
      return msg.reply('This command can only be run on Verification Plus!')
    }
    ...
    return msg.reply(`Your Verification Plus access has been transferred to user ${args.user}! If you wish to take access back yourself, just run the transfer command with yourself as the user.`)
  }
}
