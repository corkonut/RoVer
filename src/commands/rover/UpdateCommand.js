...
class UpdateCommand extends Command {
  ...
  hasPermission (msg) {
    return this.client.isOwner(msg.author) || msg.member.hasPermission(this.userPermissions) || msg.member.roles.find(role => role.name === 'Verification Admin') || msg.member.roles.find(role => role.name === 'Verification Updater')
  }
  ...
  async fn (msg, args) {
    ...
    if (!(target instanceof Role)) { // They want to update a specific user (roles have .hoist, users do not)
      ...
      return msg.reply('Sorry, updating more than one user is only available with Verification Plus: NOT DONE">.')
      ...
      if (affectedCount > limit) {
        return msg.reply(`Sorry, but Verification only supports updating up to ${limit} members at once. Updating this role would affect ${affectedCount} members.`)
      }
